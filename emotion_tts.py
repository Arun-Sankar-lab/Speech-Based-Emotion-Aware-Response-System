...
def extract_emotion_embeddings(emotion_model_path='emotion_model.keras'):
    try:
...
class EmotionTTSEngine:
    def __init__(self, emotion_model_path='emotion_model.keras', tts_model_path='emotion_tts_gan'):
        ...
        if self.emotion_embeddings is not None:
            print(f" Using YOUR model's learned emotion representations!")
        else:
            embed_dim = 256
            print(f"Using random embeddings")
        
        print("\n[2/3] Building TensorFlow GAN models...")
        self.conditioner = build_emotion_conditioner(embed_dim)
        self.vocoder = build_hifigan_generator()
    ...
    def _apply_gan_conditioning(self, audio, emotion):
        try:
            ...
            print(f"GAN conditioning: scale {dsp_scale:.2f}")
...
def speak_response(text, emotion='neutral', emotion_model_path='emotion_model.keras'):
    global _engine
    if _engine is None:
